...
print("building GRAVEWATER — CRUSADE (II) ...")
...
# FAST CYCLES: build -> charge -> creep break. ROTATING risers + ostinato
# variants so each cycle is familiar-but-fresh (repetition-with-variation).
build_bars(4, 2, kind="dread")
charge(6, 6, 1.0, chant_phrase=CHANT_A, chant_oct=0, hit="braam", ostv=0)     # cycle 1
creep_break(12, 2, chant_phrase=None)

build_bars(14, 2, kind="toll")
charge(16, 6, 1.08, chant_phrase=CHANT_B, chant_oct=0, hit="anvil", ostv=1)   # cycle 2 (gallop groove)
creep_break(22, 2, chant_phrase=CHANT_A[:4])

build_bars(24, 2, kind="roll")
charge(26, 6, 1.12, chant_phrase=None, hit="braam", alien=True, ostv=2)       # cycle 3 (8ths+octaves, chant rests)
for b in range(26, 32, 2):
    war_horn(CYCLE_ROOT[b % 4], 0.5, b, 0, 0.3)

# §THE EYE — 30s eerie centerpiece, dead centre (bars 32-43 ≈ 80-110 s)
centerpiece(32, 12)

# §CLIMAX (44-51): erupts out of the Eye; chant C + choir + horns; peak ~bar 46 (~115s)
charge(44, 8, 1.24, chant_phrase=CHANT_C, chant_oct=1, hit="braam", ostv=1, counter=True)
chant(CHANT_C, 44, 0.32, octv=0)
for nm in CHORD["D"][2:]:
    CHOIR.add(choir_pad(note(nm) * 2, 8 * BAR, 0.22), bt(44), 1.0, 0.5, 0.32)
for b in range(44, 52, 2):
    war_horn(CYCLE_ROOT[b % 4], 0.6, b, 0, 0.34)
for nm in ["D3", "Ab3"]:                                              # tritone dread stab
    MUS.add(brass(note(nm), 0.7, 0.3), bt(46), 1.0, 0.5, 0.2)
PAL.add(choir_stab(0.4), bt(46), 1.0, 0.5, 0.3)                       # #8 CHOIR STAB at apex

# §COLLAPSE (52-53): sudden strip -> sub-drop + dark whoosh
DRUM.add(subdrop(0.7), bt(52), 1.0, 0.5, 0.08)                        # #2 SUB DROP
DRUM.add(dark_whoosh(2 * BAR, 0.4), bt(52), 1.0, 0.5, 0.2)           # #9 DARK WHOOSH
PAL.add(groundwater(6, 36.7, 30.0, 0.4), bt(52), 1.0, 0.5, 0.25)
PAL.add(chorus_many(6, NOMINAL, 0.38), bt(52), 1.0, 0.5, 0.4)
toll(52, PRIME, 0.4)

# §FINAL CHARGE (54-63): biggest, relentless, chant C + B
build_bars(53, 1, kind="ghost")
charge(54, 10, 1.26, chant_phrase=CHANT_C, chant_oct=1, hit="braam", ostv=1, counter=True)
for ph in (54, 60):
    chant(CHANT_B, ph, 0.36, octv=0, vary=True)
    for b in range(ph, ph + 6, 2):
        war_horn(CYCLE_ROOT[b % 4], 0.6, b, 0, 0.34)
for nm in CHORD["D"][2:]:
    CHOIR.add(choir_pad(note(nm) * 2, 10 * BAR, 0.2), bt(54), 1.0, 0.5, 0.32)

# §REQUIEM OUTRO (64-71): organ fades, lone chant, toll, sink, gong, caught breath
...
PAL.add(gong(0.4), bt(66), 1.0, 0.5, 0.45)
chant(CHANT_A, 65, 0.34, octv=0, vary=True)
# CALLBACK / peak-end: the Eye's 'sigh' motif returns bare as the last melodic
# gesture (ends on the hanging leading-tone C# -> unresolved), + inhaling bell.
for nm, bb, d, vw in EYE_MOTIF:
    CHOIR.add(chant_note(note(nm), d * BEAT * 0.95, vw, 0.34), bt(68, bb), 1.0, 0.5, 0.5)
PAL.add(cold_bell_larynx(6, QUINT, 0.3), bt(68), 1.0, 0.5, 0.45)
toll(66, 73.4, 0.5); toll(71, 55.0, 0.5)
PAL.add(pleura(18, 0.5, caught=True), bt(69), 1.0, 0.5, 0.15)
...
def loudness(n):
    # fast cycles (swings) -> deep EYE dip in the middle -> climax -> collapse -> final
    pts = [(0, -19), (10, -14), (20, -6), (30, -12), (40, -6), (50, -13), (65, -5),
           (78, -10),                     # end of cycle 3
           (82, -18), (88, -17),          # EYE: void + exposed solo (very quiet)
           (96, -13), (104, -8), (110, -5),   # EYE: awakening -> launch (rising dread)
           (114, -4),                     # climax erupts
           (128, -5), (133, -13),         # collapse dip
           (140, -4), (158, -6),          # final charge
           (165, -12), (184, -40)]        # requiem fade
