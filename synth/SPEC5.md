All ground truth verified. Key confirmations that shape the spec:

- `synth/residua.py` and `synth/qa.py` **do not exist** → build starts with `cp crusade2.py residua.py`; `qa.py` must be created.
- `engine.py` `i_lead`/`i_pluck`/`play_hook`/`play_hook_pluck` are **nested inside `render()`** (line 328+), depending on render-local `music`/`pump`/`add_wide`/`st`/`supersaw`/`delay` → **un-importable**. Confirmed trap.
- `orch.note()` parses `F5`/`Eb5`/`F6` correctly (verified the parser). `chant_note()` has **no `cents` param** → microtonal lean must multiply the frequency.
- `cribra()` returns **stereo `(L,R)`** → must use `add_st`. `vox_glottis`, `_additive`, `vib` exist but are **not** in crusade2's import lines.
- crusade2's `master()` already has the sidechain (`sc_env`), de-harsh notch, dark tilt, `osat`, tanh limiter — reusable verbatim; only the 3-tier wiring + LEAD bus need adding.
