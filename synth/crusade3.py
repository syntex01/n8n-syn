...
def build_bars(b0, nbars, horn=True):
    """Short build: snare roll + DREAD RISER (no charge-up) + reverse-swell cut."""
    snare_roll(b0, nbars, 0.5)
    DRUM.add(dread_riser(nbars * BAR, 0.5), bt(b0), 1.0, 0.5, 0.22)
    DRUM.add(rev_swell(nbars * BAR, 0.38), bt(b0), 1.0, 0.5, 0.25)
    if horn:
...
# FAST CYCLES (first half): build -> charge (with braam/anvil hits) -> creep break
build_bars(4, 2)
charge(6, 6, 1.0, chant_phrase=CHANT_A, chant_oct=0, hit="braam")     # cycle 1
creep_break(12, 2, chant_phrase=None)

build_bars(14, 2)
charge(16, 6, 1.08, chant_phrase=CHANT_B, chant_oct=0, hit="anvil")   # cycle 2
creep_break(22, 2, chant_phrase=CHANT_A[:4])

build_bars(24, 2)
charge(26, 6, 1.12, chant_phrase=None, hit="braam", alien=True)       # cycle 3 (chant rests)
...
charge(44, 8, 1.24, chant_phrase=CHANT_C, chant_oct=1, hit="braam")
...
build_bars(53, 1)
charge(54, 10, 1.26, chant_phrase=CHANT_C, chant_oct=1, hit="braam")
for ph in (54, 60):
